...
func InitData() {
	...
	defer db2.Close()
	InitTable(db2, Table2, nil)

	var entity Entity
	for rows.Next() {
		_ = common.TryCatch(rows.Scan(&entity.Id, &entity.Text, &entity.Date), &common.ErrMsg{}, false, DB, db2)
		_ = common.RunQuery(db2, fmt.Sprintf("INSERT INTO %s(id, text, date) VALUES($1, $2, $3);", Table2), false, entity.Id, entity.Text, entity.Date)
	}
}
